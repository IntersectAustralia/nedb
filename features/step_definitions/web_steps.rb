...
When /^(?:|I )follow exact link "([^"]*)"$/ do |link|
  link = find_with_exact_text('a', link)
  link.click
end
...
Then /^the "([^"]*)" field should have the error "([^"]*)"$/ do |field, error_message|
  ...
  if page.respond_to?(:should)
    if using_formtastic
      ...
      page.should have_content("#{field} #{error_message}")
    end
  else
    if using_formtastic
      error_paragraph = element.find(:xpath, '../*[@class="inline-errors"][1]')
      ...
      assert page.has_content?("#{field} #{error_message}")
    end
  end
end
...
def find_with_exact_text(element, text)
  matches = all(element, :text => text)
  exact = matches.find_all { |match| match.text.strip == text}
  raise "Multiple elements with exact text #{text}" if exact.count > 1
  raise "No element with exact text #{text}" if exact.count < 1
  exact.first
end
