...
Feature: Edit Confirmation
  ...
  Background:
    ...
    And the specimen's determination has confirmation
      | confirmer              | H.C. Smith                  |
      | confirmation_date_year | 2010                        |
      | confirmers_herbarium   | SCU                         |
  ...
  Scenario: Edit a confirmation
    When I follow "Edit confirmation"
  ...
  Scenario: Edit a confirmation by using the auto complete
    When I follow "Edit confirmation"
    And I click on the confirmers select box
    ...
    And I fill in "confirmation_confirmation_date_year" with "2011"
    And I click on the confirmer's herbarium select box
  ...
  Scenario: Edit confirmation with validation issues
    When I follow "Edit confirmation"
    And I click on the confirmers select box
    And I select "Please select" from the confirmers select
    And fill in "confirmation_confirmation_date_year" with ""
    And press "Update Confirmation"
